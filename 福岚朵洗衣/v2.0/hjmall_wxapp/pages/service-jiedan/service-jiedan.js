...
Page({
    ...
    onLoad: function (options) {
        ...
        this.loadCategory(), this.loadShape()
        if (options.goodsData) {
            this.setData({
                currentgoodsData: JSON.parse(options.goodsData)
            })
        }
    },

    loadCurrentGoods() {
        let tempGoods = this.data.currentgoodsData
        let tempFormList = this.data.formList
        tempFormList[tempGoods.currentPageCheckIndex].currentGood = tempGoods.currentPageCheckItem
        tempFormList[tempGoods.currentPageCheckIndex].attr = tempGoods.currentAttrList
        this.setData({
                    ...
                    app.core.uploadFile({
                        ...
                        complete: (t) => {
                            ...
                            if (t.data) {
                                ...
                                tempFormList[currentIndex].tempImageSrc.push(result.data.url)
                                this.setData({
                                    formList: tempFormList
                                })
                            }
    ...
    // 放大图片
    showImage(event) {
        let currentIndex = event.currentTarget.dataset.index1
        let imgUrl = this.data.formList[currentIndex].tempImageSrc;
        let preViewImg = this.data.formList[currentIndex].tempImageSrc[event.currentTarget.dataset.index2]
        // let array = []
        // array.push(imgUrl + '')
        wx.previewImage({
            urls: imgUrl, //需要预览的图片http链接列表，注意是数组
            current: preViewImg, // 当前显示图片的http链接，默认是第一个
        })
    },
    ...
    deleteImage(event) {
        let currentIndex = event.currentTarget.dataset.index1
        let currentImgIndex = event.currentTarget.dataset.index2
        let tempFormList = this.data.formList
        tempFormList[currentIndex].tempImageSrc.splice(currentImgIndex, 1)
        this.setData({
            formList: tempFormList
        })
    },
    ...
    createOrder() {
        ...
        //         })
        //         flag = false
        //     }else if (Object.keys(item.currentGood).length==0) {
        //         wx.showModal({
        //             title: '提示',
        //             content: '您有条目暂未选择商品,请检查',
        //             showCancel: false
        //         })
        //         flag = false
        //     }
        // })
        for (let item in tempFormList) {
            if (!tempFormList[item].codeValue) {
                wx.showModal({
                    title: '提示',
                    content: '您有条目暂未填写条形码,请检查',
                    showCancel: false
                ...
                flag = false
                break;
            } else if (Object.keys(tempFormList[item].currentGood).length == 0) {
                wx.showModal({
                    title: '提示',
                    content: '您有条目暂未选择商品,请检查',
                    showCancel: false
    ...
    createOrder2() {
        ...
        for (let item of tempArray) {
            if (item.addShape) {
                flag3 = !0 // 表示选择了配件
                for (let item2 of item.addShape) {
                    allShapeArr.push(item2)
                }
            }
        }
        if (flag3) {
            for (let i=0;i<allShapeArr.length;i++){
                if (allShapeArr[i] == allShapeArr[i+1]){
                    flag = !1
                    break
                }
            }
        ...
        tempArray.forEach(item => {
            if (Number(item.shape) < Number(this.data.shapeStart) || Number(item.shape) > Number(this.data.shapeEnd)) {
                flag2 = !1
            }
        })
        if (flag) {
            if (flag2) {
                ...
                app.request({
                    url: app.api.logist.torder,
                    method: 'POST',
                    data: {
                        shape_list: flag3? JSON.stringify(allShapeArr): JSON.stringify(shapeList)
                    },
                    success: res2 => {
                        if (0 == res2.code) {
                            if (flag3){ // 有配件
                                ...
                                tempArray.forEach(item => {
                                    ...
                                    if(item.addShape){
                                        for (let item2 of item.addShape){
                                            tempShape.push(item2)
                                        }
                                    }
                                    console.log([item.shape].concat(tempShape));
                                    this.addCart(item.goods_id, item.attr, [item.shape].concat(tempShape), item.tempSrc)
                                })
                            }else{
                                tempArray.forEach(item => {
                                    this.addCart(item.goods_id, item.attr, [item.shape], item.tempSrc)
                                })
                            }
                            setTimeout(() => {
                                let s = []
    ...
    loadShape() {
        ...
        app.request({
            url: app.api.logist.bcode,
            method: 'POST',
            success: res => {
                this.setData({
                    shapeStart: res.data.start,
                    shapeEnd: res.data.end
                })
            },
            complete: () => {
                wx.hideLoading()
    ...
    loadCategory() {
        ...
        app.request({
            url: app.api.default.goods_list,
            success: res => {
                this.setData({
                    goods: res.data.list,
                ...
                let tempBbj = {
                    ...
                    attr: [],
                    tempImageSrc: [],
                    addShape: undefined, // 配件的条码
                }
                formList.push(tempBbj)
                this.setData({
                    formList: formList
                })
                if (this.data.currentgoodsData) {
                    this.loadCurrentGoods()
                }
            },
            complete: function () {
                wx.hideLoading()
    ...
    addCart(goodsId, attr, shape, photo) {
        AllFalg = !0
        // let attr = []
        // attr.push({
    ...
    onShow: function () {
        ...
        // console.log(currPage.data.formList);
        // let tempObj = wx.getStorageSync('checkGoodsInfo')
        // if (tempObj && Object.keys(tempObj).length!=0){
        //     let tempList = currPage.data.formList
        //     tempList[Number(tempObj.currentPageCheckIndex)].currentGood = tempObj.currentPageCheckItem
        //     tempList[Number(tempObj.currentPageCheckIndex)].attr =  tempObj.currentAttrList
        //     currPage.setData({
        //         formList: tempList
        //     })
        // }
        ...
        if (currPage.data.currentPageCheckItem) {
            ...
            tempList[index].attr = currPage.data.currentAttrList
            tempList[index].addShape = currPage.data.currentPageAddShape || undefined
            setTimeout(() => {
                this.setData({
                    formList: tempList
                })
