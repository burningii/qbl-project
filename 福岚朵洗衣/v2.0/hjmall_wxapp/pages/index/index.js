...
Page({
    data: {
        ...
        time_all: [],
        navH: '',
        curentUserAvatar: 'https://s2.ax1x.com/2019/09/27/uKKzL9.png',
        roleFalg: null,
        navBarTitle: '',
        showQrCodeVisible: false,
        qrCodeSrc: '', // 二维码图片路径
        navigateBarColor:'#FD7237' // 顶部导航栏颜色
    },
    onLoad: function(t) {
        // swiper设置高度
        var that = this;
        ...
        that.setData({
            ...
            curentUserAvatar: wx.getStorageSync('USER_INFO').avatar_url
        })
    ...
    goInfoOrder(e){
        ...
        wx.navigateTo({
          url: `/pages/wuliu-status-order/index?f=${f}`
        })
    },
    ...
    service_jiedan(){
        wx.navigateTo({
          url: '/pages/service-jiedan/service-jiedan'
        })
    },
    // 物流打包页面
    wuliu_dabao(){
        // wx.navigateTo({
        //     url: '/pages/wuliu-dabao/wuliu-dabao'
        // })
        wx.navigateTo({
            url: '/pages/navto-scan5/navto-scan'
        })
    },
    // 收件点送件(出示二维码给物流扫)
    service_songjian(){
    ...
    service_songjian2(){
        wx.navigateTo({
          url: '/pages/wuliu-factory/wuliu-factory'
        })
    },
    service_songjian3(){
        wx.navigateTo({
          url: '/pages/wuliu-factory/wuliu-factory?flag=1'
        })
    },
    // 收件点扫物流的码(取回衣物)
    service_jiejian(){
    ...
    wuliu_shouJian(){
        wx.navigateTo({
          url: `/pages/wuliu-shoujian/index`
        })
        // // 只允许从相机扫码
        // wx.scanCode({
        //     onlyFromCamera: true,
    ...
    // 物流工厂取件(通过条形码)
    wuliu_factory_jiejian(){
        // wx.navigateTo({
        //   url: '/pages/factory-take/factory-take'
        // })
        wx.navigateTo({
            url: '/pages/navto-scan3/navto-scan'
        })
    },
    // 展示工厂的接件所有订单页面
    factoryShowOrder(){
        wx.navigateTo({
          url: '/pages/factory-order/factory-order'
        })
    },
    service_chuku(){
        wx.navigateTo({
            url: '/pages/qrcode-page/qrcode-page'
        })
    },
    onClose(){
        this.setData({
            ...
            success: function(t) {
                if (0 == t.code) {
                    if ("diy" == t.data.status) {
                        ...
                        let template = t.data.template
                        template.forEach(item=>{
                            'search'==item.type && i.setData({
                                showVisible: !0
                            })
                        })
                    ...
                    i.data.showVisible && app.request({
                        ...
                        success: res=>{
                            ...
                            if (2==res.data.part_id){ // 物流員
                                i.setData({
                                    roleFalg: 2,
                                    navBarTitle: '物流员',
                                    navigateBarColor: '#23AFEA'
                                })
                                loadWuliuData()
                            }else if (1==res.data.part_id){ // 收件點
                                i.setData({
                                    roleFalg: 1,
                                    navBarTitle: '商户端',
                                })
                                update()
                            }else if (3==res.data.part_id){ // 工廠
                                i.setData({
                                    roleFalg:3,
                                    navBarTitle: '工厂'
                                })
                            ...
                            function loadWuliuData() {
                                wx.showLoading({
                                    title:'加载中',
                                    mask: !0
                                })
                                app.request({
                                    url:app.api.logist.figure,
                                    method: 'POST',
                                    success: res2=>{
                                        0==res.code? (i.setData({
                                            wuliuData: res2.data
                                        })):wx.showModal({
                                          title: '提示',
                                          content: res2.msg,
