module.exports = {
    ...
    showDialogBtn: function(t) {
        ...
        getApp().request({
            ...
            success: function(t) {
                if (0==t.code){
                    let myList = t.data.attr_group_list.filter(item=>{
                        return '尺码'==item.attr_group_name
                    })
                    console.log('---myList---',myList);
                    myList[0].attr_list.forEach(item=>{
                        item.attr_nums = 0
                    })
                    let newAttrGroupList=t.data.attr_group_list.filter(item=>{
                        return '尺码'!=item.attr_group_name
                    })
                    newAttrGroupList[0].attr_list[0].checked=!0
                    newAttrGroupList[0].attr_list[0].is_attr_num=!1
                    r.setData({
                        currentGood: t.data,
                        goods_name: t.data.name,
                        // attr_group_list: t.data.attr_group_list,
                        attr_group_list: newAttrGroupList,
                        sizeAttrList: myList,
                        showModal: !0
        ...
        for (var e in i) {
            ...
            for (var d in a) {
                ...
                if (n.sort().join() === c.sort().join()) {
                    ...
                    console.log('----参数----',o);
                    let myList = o.filter(item=>{
                        return '尺码'!=item.attr_group_name
                    })
                    return void t.setData({
                        // attr_group_list: o,
                        // attr_group_list: myList,
                        check_num: a[d].num,
    ...
    attrClick: function(t) {
        ...
        let {checkedAttrList, sizeAttrList} = r.data
        let result = checkedAttrList.find(item => {
            return item.group_id == t.currentTarget.dataset.id
        })
        const detailData = () => {
            for (let m = 0; m < sizeAttrList.length; m++) {
                for (let j = 0; j < sizeAttrList[m].attr_list.length; j++) {
                    sizeAttrList[m].attr_list[j].attr_nums = 0
                }
                break
            }
        }
        ...
        const reloadData = ()=>{
            if (result.list.length==0){
                sizeAttrList[0].attr_list.forEach(item=>{
                    item.attr_nums=0
                })
            }else{
                sizeAttrList[0].attr_list.forEach(item=>{
