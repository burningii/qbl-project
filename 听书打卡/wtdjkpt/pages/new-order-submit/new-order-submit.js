...
Page({
    ...
    onLoad: function(t) {
      ...
      return  item.__route__ =='pages/artificerList/artificerList'
      })
      if(page.length>0)
      this.setData({
        type:page[0].data.type
      })
      console.log(this.data.type,'type数据');
      if(this.data.type==2 || this.data.type==3) {
        console.log('whywhy')
        this.setData({
        arr:JSON.parse(wx.getStorageSync('data'))
      })
          this.artificer();
      }else{
    ...
    artificer(){
      ...
      getApp().request({
        ...
        data: {
          ...
          uid: JSON.parse(wx.getStorageSync('data')).uid
        },
        success: res => {
          _that.setData({
            arr: res.data
          })
          getApp().page.onLoad(_that, res);
        }
      })
    },
    // 输入真实姓名
    ...
    bindContentInput2(t){
        this.data.mch_list[t.currentTarget.dataset.index].phone = t.detail.value, this.setData({
            mch_list: this.data.mch_list
        });
    },
    KeyName: function(t) {
    ...
    orderSubmit: function(t) {
        ...
        for (var o in i) {
            ...
            if (s && 1 == s.is_form && 0 == i[o].mch_id) {
                ...
                for (var r in n) if (1 == n[r].required) if ("radio" == n[r].type || "checkbox" == n[r].type) {
                    ...
                    if (!c && e.data.type==3) return getApp().core.showModal({
                        title: "提示",
          ...
          if (1 == i.length && 0 == i[o].mch_id && 1 == i[o].offline && e.data.type == 3) ; 
            else {
              if(e.data.type!=3) e.setData({address:{id:0}})
        ...
        if (!e.data.realPhone || ''==e.data.realPhone){
        ...
        if (!e.data.realName || ''==e.data.realName){
            return getApp().core.showModal({
                title: "提示",
                content: "请输入您的真实姓名",
                showCancel: !1
        ...
        if(e.data.type==2 || e.data.type==3){
          console.log(e.data.arr);
          a.sub_type=e.data.type,
          a.status=e.data.type,
          a.aid=e.data.arr.id;
        };
        if(e.data.type==1) a.status=e.data.type;
        if(e.data.type==2) a.price=e.data.arr.price;
        a.name = e.data.realName
        a.phone = e.data.realPhone
        console.log('a数据',a), e.order_submit(a, "s");
    },
    onReady: function() {},
    onShow: function(t) {
        this.setData({
            realName: wx.getStorageSync('realName') || '',
            realPhone: wx.getStorageSync('realPhone') || ''
        })
        if (!is_loading_show) {
    ...
    getOrderData: function(t) {
      console.log(t,'订单数据')
        ...
        e.longitude = longitude, e.latitude = latitude, getApp().core.showLoading({
            ...
            success: function(t) {
              if (_.data.type == 2){
                console.log(_.data.arr.price);
                t.data.mch_list[0].level_price = Number(_.data.arr.price);
                t.data.mch_list[0].total_price = Number(_.data.arr.price);
                t.data.mch_list[0].goods_list[0].level_price = Number(_.data.arr.price);
                t.data.mch_list[0].goods_list[0].single_price = Number(_.data.arr.price);
                t.data.mch_list[0].goods_list[0].price = Number(_.data.arr.price);
              }
              console.log(t.data);
    ...
    cardDel: function() {
        ...
        // });
        // getApp().corde.redirectTo({
        //   url:'/pages/responseAsk/responseAsk?order_id='
        // })
    ...
    checkData(){
        ...
        wx.setStorageSync('realPhone', this.data.realPhone);
    },
    onUnload: function() {
