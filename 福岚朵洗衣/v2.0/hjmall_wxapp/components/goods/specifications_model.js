module.exports = {
    ...
    init: function(t) {
        ...
        void 0 === (a.currentPage = t).previewImage && (t.previewImage = function(t) {
            ...
            a.storeAttrClick(t);
        }), void 0 === t.storeAttrClick2 && (t.storeAttrClick2 = function(t) {
            a.storeAttrClick2(t);
    ...
    storeAttrClick2(t){
        let attr_grou_lists = this.currentPage.data.attr_group_lists
        let attr_group_list = this.currentPage.data.attr_group_list
        // attr_group_list.forEach(item=>{
        ...
        //     })
        // })
        var e = this.currentPage.data.attr_group_list, o = [];
        for (var r in e) {
            var i = !1;
        ...
        let clickGroupId = t.currentTarget.dataset.id
        let flag2=0
        attr_grou_lists.forEach(item=>{
            item.attr_lists.forEach(item2=>{
                if (clickGroupId==item2.attr_id) {
                    if (item2.checked){ // 取消
                        item2.checked=!1
                        flag2=1
                    } else{ // 选中
        ...
        this.currentPage.setData({
            attr_group_lists: attr_grou_lists
        })
        if (1==flag2){
            getApp().request({
                url: getApp().api.default.goods_attrs_info,
                data: {
                    goods_id: this.currentPage.data.id,
                    attr_list: JSON.stringify([clickGroupId]),
                    group_checked: 0,
                    type: ''
                },
                success: res=>{
                    let goods = this.currentPage.data.goods
                    goods.price = (parseFloat(goods.price)-parseFloat(res.data.price)).toFixed(2)
                    this.currentPage.setData({
                        goods:goods
                    })
                }
    ...
    storeAttrClick: function(t) {
        ...
        let attr_group_lists = e.data.attr_group_lists
        attr_group_lists.forEach(item=>{
            item.attr_lists.forEach(item2=>{
                item2.checked=!1
            })
        })
       ...
       e.setData({
            attr_group_lists: attr_group_lists
        })
