...
Page({
    ...
    orderSubmit: function(t) {
        var e = this, a = {}, i = e.data.mch_list2;
        for (var o in i) {
            var s = i[o].form;
        ...
        e.longitude = longitude, e.latitude = latitude, getApp().core.showLoading({
            ...
            success: function(t) {
                if (getApp().core.hideLoading(), 0 == t.code) {
                    ...
                    a.mch_list2 = JSON.parse(st);//s
                    const st2 = JSON.parse(st)[0].goods_list
                    const cartList = s[0].goods_list
                    const newArr = []
                    ...
                    for (let j = 0; j < newArr.length; j++) {
                        ...
                        for (let k = 0; k < st2.length; k++) {
                            if (st2[k].goods_id==newArr[j].goods_id){
                                totalPriceMy += Number(st2[k].level_price)
                            }
                        }
                        newArr[j].totalPriceMy = totalPriceMy
                    }

                    console.log(newArr,'-----处理后-----');
                    a.mch_list[0].goods_list = newArr;//s
                    var l = _.data.index;
